...
void main()
{
	...
	string nombre, edad, carnet, nacionalidad, ocupacion, horas_trabajadas, salario, fecha_vinculacion, fecha_retiro, producto, producto_vendidos, producto_total, satisfaccion;
	int op1;
	bool salir = true;

	while (salir) {
		cout << "Seleccione su rol en la empresa:" << endl;
		cout << "1. Cliente" << endl; // Cliente
		...
		case 1: // Gerente - Cliente
			...
			case 2:
				...
				cin >> nacionalidad;
				cout << "Ingrese el nivel de satisfaccion: ";
				cin >> satisfaccion;

				cliente.set_nombre(nombre);
				cliente.set_edad(edad);
				cliente.set_Carnet_identidad(carnet);
				cliente.set_nacionalidad(nacionalidad);
				cliente.set_satisfaccion(satisfaccion);
				cliente.anadir(
					"cliente",
					cliente.get_nombre(),
					cliente.get_edad(),
					cliente.get_Carnet_identidad(),
					cliente.get_nacionalidad(),
					"NULL",
					"NULL",
					"NULL",
					"NULL",
